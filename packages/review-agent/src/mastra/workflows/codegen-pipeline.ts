...
function buildCommitMessage(ctx: IssueContext, summary: string): string {
  let message = `feat: implement issue #${ctx.issueNumber}\n\nAuto-generated by InnoAgent`
  if (summary) message += `\n\n${summary}`
  return message
}
...
export async function runCodegen(
  ...
  const gitflame = getGitFlameClient()
  const branch = `innoagent-issue-${ctx.issueNumber}`

  try {
    // ── clone + branch ────────────────────────────────────────────────────
    // Fatal here, unlike review: there is no tree to generate against without
    // it, so a failure must be retried rather than degrading. A real clone
    // (not the archive snapshot review-pipeline.ts still uses) means the
    // eventual push is a clean fast-forward off a real ancestor commit.
    try {
      const cloneUrl = gitflame.getAuthenticatedCloneUrl(ctx.owner, ctx.repo)
      await cloneAndBranch(exec, { cloneUrl, defaultBranch: ctx.defaultBranch, branch })
    } catch (err) {
      throw new Error(gitflame.redactToken(err instanceof Error ? err.message : String(err)))
    }
    ...
    return { summary, branch, verified: result.ok, changedFiles }
  } finally {
