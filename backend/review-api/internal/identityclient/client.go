...
// RevokeDelegation revokes a delegation grant in identity.
// userToken is the user's Bearer token (forwarded as-is from the erase request).
// clientID is the service whose standing permission is being revoked.
func (c *Client) RevokeDelegation(ctx context.Context, userToken, clientID string) error {
	body, err := json.Marshal(map[string]string{"client_id": clientID})
	if err != nil {
		return fmt.Errorf("identity client: marshal: %w", err)
	}
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		c.baseURL+"/identity/v1/delegation-revoke", bytes.NewReader(body))
	if err != nil {
		return fmt.Errorf("identity client: build request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("identity client: delegation revoke: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()

	if resp.StatusCode != http.StatusNoContent {
		return fmt.Errorf("identity client: delegation revoke: unexpected status %d", resp.StatusCode)
	}
	return nil
}
